...
def get_data_token_count(tokenlist):
    ...
    """
    vocab = defaultdict(int) #ekta dictionary create korse
    #set CV list indicies

    for i, tokens in enumerate(tokenlist):  #tokenlist er token gula ke access korse
        vocabCounter = dict(Counter(tokens[0]))  #eikhane shudhu token 0 keno nilo? pura token hbe
...
#         vocab: counter of token occurances as ordered dict
#     """
#     vocab = defaultdict(float) #ekta dictionary create korse
#     #set CV list indicies

#     for i, tokens in enumerate(tokenlist):  #tokenlist er token gula ke access korse
#         vocabCounter = dict(Counter(tokens))  #eikhane shudhu token 0 keno nilo? pura token hbe
#         for this_token in vocabCounter.keys():
#             try: vocab[this_token] += 1
#             except KeyError: vocab[token] = 1
#     return vocab
...
def wv_initialize(pretrained_path,minDF, train_vocab,rand_state, k=300):
    ...
    if pretrained_path != None:
        with open(pretrained_path, "rb") as f:
            header = f.readline()
            vocab_size, layer1_size = map(int, header.split())  #Reads the first line to get the total number of vectors (vocab_size) and their dimensionality (layer1_size).
            binary_len = np.dtype('float32').itemsize * layer1_size #Determines the size in bytes of each word vector based on its dimensionality.
            for line in range(vocab_size): 
                word = []
                while True:
                    ch = f.read(1)  #Reads the word byte by byte until a space is encountered, signaling the end of the word.
                    if ch == ' ':
                        word = ''.join(word)
                        break
                    if ch != '\n':
                ...
                if word in train_vocab:
                   wordVecs[word] = np.fromstring(f.read(binary_len), dtype='float32')  #If the word is in the training vocabulary, its vector is read from the file and stored in wordVecs.
                else:
                    f.read(binary_len)
    #initialize words that aren't read from pre-trained
    for word in train_vocab:
        if word not in wordVecs and train_vocab[word] >= minDF:
            wordVecs[word] = rand_state.uniform(-0.25,0.25,k)   #Initializes vectors randomly for words in train_vocab not found in the pre-trained set but meet the minDF threshold.
...
def getidxWVs(loadedWV):
    ...
    wvToIdx = dict()  #a dictionary for mapping words to their indices
    k = 300
    WVmatrix = np.zeros(shape=(vocabSize+1, k), dtype='float32')  #Prepares a matrix for word vectors
...
def cnn_tokensToIdx(inTokens, wvToIdx, maxLen , pad_size, reverse_tokens):
    ...
    if reverse_tokens:
        inTokens = inTokens[::-1]
    docAsWVidx = []  #Initializes an empty list to hold the indices corresponding to the word vectors of the tokens in the input document.
    padSize = pad_size
    for i in range(padSize):
        docAsWVidx.append(0)
    for word in inTokens:
        #If the word is found in the wvToIdx mapping and the length of docAsWVidx is less than maxLen, the word's index is appended to docAsWVidx
    ...
    while len(docAsWVidx) < maxLen:
        #pad with zeros
        docAsWVidx.append(0)  #if the length of docAsWVidx is still less than maxLen, additional zeros are appended to the end until the length requirement is satisfied
